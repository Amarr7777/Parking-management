...
void WelcomeMessage() {
    printf("\n\nWelcome to Christ College Parking Management System!\n");
}
...
void Exit() {
    printf("\nEnter ID to remove the vehicle: ");
    int exitID;
    scanf("%d", &exitID);

    for (int i = 0; i < 1500; i++) {
        if (usedIDs[i] == exitID) {
            usedIDs[i] = 0;  // Mark the slot as empty
            if (i < 500) {
                nob--;
            } else if (i < 1000) {
                noc--;
            ...
            count--;
            printf("\nVehicle with ID %d removed successfully from slot %d.", exitID, i + 1);
            return;
        }
    }

    printf("\nNo vehicle found with ID %d", exitID);
}
